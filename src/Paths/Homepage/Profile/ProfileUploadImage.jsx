...
import LogoUpload from "../../../assets/images/icon-upload-image.svg"
...
const ProfileUploadImage = () => {
    ...
    return (
        <StyledUploadComponent>
            <input className={`upload--input ${isImageUploaded ? "uploaded" : ""}`} type="file"
                   accept="image/jpeg, image/png, image/jpg" onChange={(event) => {
                handleChange(event)
            }}></input>
            ...
            <output>
                {isImageUploaded &&
                    <img src={uploadedImage} alt="uploaded" className={`${isImageUploaded ? "uploaded--image" : ""}`}/>}
            </output>
        </StyledUploadComponent>
    )
}
